...
# re-sign to actually finish, then send ONE push notification with the real
# success/failure result (unlike atvloadly's built-in scheduler, which only
# notifies on failure). Intended to be driven by atvloadly-refresh.timer.
set -uo pipefail
...
[ -z "$SID" ] && { echo "[$(TS)] ERROR: no MCP session" >> "$LOG"; notify "atvloadly refresh FAILED" "no MCP session"; exit 1; }
...
[ -z "$IDS" ] && { echo "[$(TS)] ERROR: no apps" >> "$LOG"; notify "atvloadly refresh FAILED" "no enabled apps"; exit 1; }
...
print("atvloadly refresh: %s ok / %s failed" % (s.get("success_count"), s.get("failed_count")))
print(" | ".join(lines))
')
TITLE=$(printf '%s\n' "$REPORT" | sed -n '1p')
BODY=$(printf  '%s\n' "$REPORT" | sed -n '2p')
[ -z "$TITLE" ] && TITLE="atvloadly refresh: status unknown (timeout)"
notify "$TITLE" "$BODY"
